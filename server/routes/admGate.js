...
import { isGateEnabled } from '../services/adminerGate.js';

const router = Router();

const GATE_COOKIE = 'cd_adm_gate';
const GATE_TTL_MS = 24 * 60 * 60 * 1000;

// Bramka Turnstile dla Adminera - ten sam mechanizm co server/routes/
// pmaGate.js (bramka phpMyAdmin), osobne ciasteczko (cd_adm_gate) i
// osobny wlacznik (server/services/adminerGate.js), zeby dwie bramki
// dzialaly niezaleznie od siebie. Zamontowany POZA `/api` w
// server/index.js z tego samego powodu co pmaGate.js - odwiedzajacy
// Adminera nigdy nie logowal sie do panelu.
...
<body>
  ...
  <script>
    window.__admGateCallback = async function(token) {
      var errEl = document.getElementById('err');
      errEl.textContent = 'Weryfikuje...';
      try {
        var res = await fetch('/adm-gate/verify', {
          method: 'POST',
    ...
    (function poll() {
      if (window.turnstile) {
        window.turnstile.render('#turnstile-container', {
          ...
          theme: 'auto',
          callback: window.__admGateCallback,
