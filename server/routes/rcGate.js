...
import { getRoundcubeConfig } from '../services/roundcubeConfig.js';

const router = Router();

const GATE_COOKIE = 'cd_rc_gate';
const GATE_TTL_MS = 24 * 60 * 60 * 1000;

// Bramka Turnstile dla Roundcube - ten sam mechanizm co server/routes/
// pmaGate.js (bramka phpMyAdmin)/admGate.js (Adminer), osobne ciasteczko
// (cd_rc_gate). W odroznieniu od tamtych dwoch, wlacznik NIE jest
// osobnym plikiem (services/roundcubeGate.js) - Roundcube juz ma wlasny
// stan (domain/gateEnabled) w services/roundcubeConfig.js, wiec
// isGateEnabled tutaj to po prostu odczyt tego samego pola. Zamontowany
// POZA `/api` w server/index.js z tego samego powodu co pmaGate.js -
// odwiedzajacy webmail.<domena> nigdy nie logowal sie do panelu.
function isGateEnabled() {
  return getRoundcubeConfig().gateEnabled;
}
...
<body>
  ...
  <script>
    window.__rcGateCallback = async function(token) {
      var errEl = document.getElementById('err');
      errEl.textContent = 'Weryfikuje...';
      try {
        var res = await fetch('/rc-gate/verify', {
          method: 'POST',
    ...
    (function poll() {
      if (window.turnstile) {
        window.turnstile.render('#turnstile-container', {
          ...
          theme: 'auto',
          callback: window.__rcGateCallback,
