...
# Idempotentne - jesli nasz socket juz tam jest, nic nie zmieniamy.
for PARAM in smtpd_milters non_smtpd_milters; do
  CURRENT="$(postconf -h "$PARAM" 2>/dev/null || true)"
  if [[ "$CURRENT" != *"${SPAMASS_MILTER_SOCKET}"* ]]; then
    if [ -n "$CURRENT" ]; then
      postconf -e "${PARAM}=${SPAMASS_MILTER_SOCKET}, ${CURRENT}"
    else
      postconf -e "${PARAM}=${SPAMASS_MILTER_SOCKET}"
    fi
  fi
done
