...
const SCRIPT_PATH = path.join(path.dirname(fileURLToPath(import.meta.url)), '../scripts/mongodb-test-db.sh');
...
async function getTestDbStatus() {
  const { message } = await runAction('status', 'testowej bazy MongoDB');
  return { exists: message.trim() === 'exists' };
}

async function createTestDb() {
  return runAction('create', 'testowej bazy MongoDB');
}

async function dropTestDb() {
  return runAction('drop', 'testowej bazy MongoDB');
}
