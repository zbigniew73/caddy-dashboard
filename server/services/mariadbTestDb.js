...
const SCRIPT_PATH = path.join(path.dirname(fileURLToPath(import.meta.url)), '../scripts/mariadb-test-db.sh');
...
async function getTestDbStatus() {
  const { message } = await runAction('status', 'testowej bazy MariaDB');
  return { exists: message.trim() === 'exists' };
}

async function createTestDb() {
  return runAction('create', 'testowej bazy MariaDB');
}

async function dropTestDb() {
  return runAction('drop', 'testowej bazy MariaDB');
}
